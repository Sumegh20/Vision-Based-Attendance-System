...
class FacePredictor:
    def __init__(self):
        try:
            ...
            self.embeddings = "faceEmbeddingModel/embeddings.pickle"
            self.le = "faceEmbeddingModel/label_encoder.pickle"
            self.train_model = "faceEmbeddingModel/my_model.h5"

            # Load embeddings and labels
            self.data = pickle.loads(open(self.embeddings, "rb").read())
            self.label_data = pickle.loads(open(self.le, "rb").read())

            self.embeddings = np.array(self.data['embeddings'])
            self.labels = self.label_data.fit_transform(self.data['names'])

            # Load the classifier model
            self.model = load_model(self.train_model)
    ...
    def CosineSimilarity(test_vec, source_vecs):
        ...
        for source_vec in source_vecs:
            cos_distance += FacePredictor.findCosineDistance(test_vec, source_vec)

        return cos_distance/len(source_vecs)
    ...
    def detectFace(self):
        try:
            ...
            while True:
                ret, frame = cap.read()
                frames += 1
                frame = cv2.resize(src=frame, dsize=(save_width, save_height))
                ...
                if frames % 3 == 0:
                    ...
                    if len(bboxes) != 0:
                        for bbox_i in bboxes:
                            ...
                            # Get highest accuracy embedded vector
                            j = np.argmax(preds)
                            proba = preds[j]

                            # Compare this vector to source class vectors to verify it is actual belong to this class
                            match_class_index = (self.labels == j)
                            match_class_index = np.where(match_class_index)[0]
                            selected_index = np.random.choice(match_class_index, self.comparing_num)
                            compare_embeddings = self.embeddings[selected_index]
                            ...
                            if cos_similarity < self.cosine_threshold and proba > self.proba_threshold:
                                name = self.label_data.classes_[j]
                                text = f"{name}"
                            ...
                            cv2.putText(frame, text, (bbox[0], y), cv2.FONT_HERSHEY_SIMPLEX, 2, (225, 225, 225), 2)
                            cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (179, 0, 149), 4)

                            if text != 'Unknown' and not isRecorted:
                                self.markAttendance(text)
                                isRecorted = True
                    ...
                    for tracker, text in zip(trackers, texts):
                        ...
                        cv2.putText(frame, text, (startX, startY - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.95, (255, 255, 255), 2)
            ...
            raise Exception(f"(detectFace): Something went wrong in detectFace\n" + str(e))
